...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `matohmat`.`products_all` (`prod_id` INT, `price` INT, `name` INT, `reorder_point` INT, `image_url` INT, `available` INT);
...
CREATE PROCEDURE `product_add` (
    ...
    IN items_crate INT)
BEGIN
	INSERT INTO product_detail( name, image_url, reorder_point, available, product_hash, items_per_crate )
    VALUES (product_name, image_url, reorder, TRUE, phash, items_crate);
